...
def test_load_valid_config(create_temp_config_file: Callable[[dict[str, t.Any]], str]) -> None:
    ...
    loaded_params, _ = load_named_server_configs_from_file(tmp_config_path, base_env)
...
def test_load_config_with_not_enabled_server(
    ...
    tmp_config_path = create_temp_config_file(config_content)
    loaded_params, _ = load_named_server_configs_from_file(tmp_config_path, {})
...
def test_load_example_fetch_config_if_uvx_exists() -> None:
    ...
    loaded_params, _ = load_named_server_configs_from_file(example_config_path, base_env)
...
def test_load_overrides(create_temp_config_file: Callable[[dict[str, t.Any]], str]) -> None:
    """Test loading tool overrides."""
    config_content = {
        "mcpServers": {
            "s1": {"command": "true"},
        },
        "overrides": {
            "tool1": {
                "rename": "renamed_tool1",
                "description": "new desc",
                "defaults": {"arg1": "val1"},
                "hide_fields": ["arg2"],
            },
            "tool2": {
                "rename": "renamed_tool2",
            }
        }
    }
    tmp_config_path = create_temp_config_file(config_content)
    _, overrides = load_named_server_configs_from_file(tmp_config_path, {})

    assert "tool1" in overrides
    assert overrides["tool1"]["rename"] == "renamed_tool1"
    assert overrides["tool1"]["description"] == "new desc"
    assert overrides["tool1"]["defaults"] == {"arg1": "val1"}
    assert overrides["tool1"]["hide_fields"] == ["arg2"]

    assert "tool2" in overrides
    assert overrides["tool2"]["rename"] == "renamed_tool2"
